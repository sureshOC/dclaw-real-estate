...
async def seed_demo_data() -> None:
    async with AsyncSession(engine, expire_on_commit=False) as session:
        ...
        vendors = [
            Vendor(name="QuickFix Plumbing", specialty=VendorSpecialty.plumbing, phone="555-0101", email="quickfix@example.com", rating=4.8),
            Vendor(name="BrightSpark Electric", specialty=VendorSpecialty.electrical, phone="555-0102", email="bright@example.com", rating=4.5),
            Vendor(name="CoolAir HVAC", specialty=VendorSpecialty.hvac, phone="555-0103", email="coolair@example.com", rating=4.7),
            Vendor(name="HandyPro General", specialty=VendorSpecialty.general, phone="555-0104", email="handy@example.com", rating=4.2),
            Vendor(name="GreenThumb Landscaping", specialty=VendorSpecialty.landscaping, phone="555-0105", email="green@example.com", rating=4.6),
            Vendor(name="RoofRight Co", specialty=VendorSpecialty.roofing, phone="555-0106", email="roof@example.com", rating=4.4),
        ]
        for v in vendors:
            session.add(v)
        ...
        props = []
        for title, addr, city, state, zipcode, price, ptype, beds, baths, sqft, status in prop_specs:
            p = Property(
                id=uuid.uuid4(), org_id=org_id, title=title, address=addr,
        ...
        tenants = []
        for first, last, email, phone, prop_idx, ls, le, rent in tenant_specs:
            t = Tenant(
                id=uuid.uuid4(), org_id=org_id, first_name=first, last_name=last,
        ...
        for prop_idx, tenant_idx, title, desc, prio, status in mr_specs:
        ...
        expense_cats = [ExpenseCategory.insurance, ExpenseCategory.tax,
